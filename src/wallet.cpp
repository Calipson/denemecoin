...
#include "denemecoin.h"    // For DecodeNameScript in GetAmounts to correctly compute credit/debit
...
{
  if (nVersion != DENEMECOIN_TX_VERSION)
    return false;
...
string CWallet::SendMoneyToBitcoinAddress(string strAddress, int64 nValue, CWalletTx& wtxNew, bool fAskFee)
{
    ...
    if (!scriptPubKey.SetBitcoinAddress(strAddress))
        return _("Invalid denemecoin address");
