...
uint256 hashDenemecoinGenesisBlock("000000000062b72c5e2ceb45fbc8587e807c155b0da735e6483dfba2f0a9c770");

class CDenemecoinHooks : public CHooks
{
public:
...
string SendMoneyWithInputTx(const CScript& scriptPubKey, int64 nValue, int64 nNetFee, const CWalletTx& wtxIn, CWalletTx& wtxNew, bool fAskFee)
{
    ...
    if (fAskFee && !uiInterface.ThreadSafeAskFee(nFeeRequired))
        return "ABORTED";
#else
    if (fAskFee && !ThreadSafeAskFee(nFeeRequired, "Denemecoin", NULL))
        return "ABORTED";
#endif
...
Value sendtoname(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoname <denemecoinname> <amount> [comment] [comment-to]\n"
    ...
    if (!AddressToHash160(strAddress, hash160))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No valid denemecoin address");
...
Value name_firstupdate(const Array& params, bool fHelp)
{
    ...
    CWalletTx wtx;
    wtx.nVersion = DENEMECOIN_TX_VERSION;
  ...
  if (!IsValidBitcoinAddress (address))
    {
      ...
      msg << "Invalid Denemecoin address: " << address;
      throw JSONRPCError (RPC_INVALID_ADDRESS_OR_KEY, msg.str ());
    }

  tx.nVersion = DENEMECOIN_TX_VERSION;
...
bool CNameDB::ReconstructNameIndex()
{
    ...
    CRITICAL_BLOCK(pwalletMain->cs_mapWallet)
    {
        while (pindex)
        {
            ...
            {
                if (tx.nVersion != DENEMECOIN_TX_VERSION)
                    continue;
...
CHooks* InitHook()
{
    ...
    hashGenesisBlock = hashDenemecoinGenesisBlock;
    printf("Setup Denemecoin genesis block %s\n", hashGenesisBlock.GetHex().c_str());
    return new CDenemecoinHooks();
}

bool CDenemecoinHooks::IsStandard(const CScript& scriptPubKey)
{
    return true;
}
...
void CDenemecoinHooks::AddToWallet(CWalletTx& wtx)
{
}

bool CDenemecoinHooks::IsMine(const CTransaction& tx)
{
    if (tx.nVersion != DENEMECOIN_TX_VERSION)
        return false;
...
bool CDenemecoinHooks::IsMine(const CTransaction& tx, const CTxOut& txout, bool ignore_name_new /* = false*/)
{
    if (tx.nVersion != DENEMECOIN_TX_VERSION)
        return false;
...
bool
CDenemecoinHooks::AcceptToMemoryPool (DatabaseSet& dbset, const CTransaction& tx)
{
    if (tx.nVersion != DENEMECOIN_TX_VERSION)
        return true;
...
void CDenemecoinHooks::RemoveFromMemoryPool(const CTransaction& tx)
{
    if (tx.nVersion != DENEMECOIN_TX_VERSION)
        return;
...
bool GetNameOfTx(const CTransaction& tx, vector<unsigned char>& name)
{
    ...
    if (!good)
        return error("GetNameOfTx() : could not decode a denemecoin tx");
...
IsConflictedTx (DatabaseSet& dbset, const CTransaction& tx, vchType& name)
{
    ...
    if (!good)
        return error("IsConflictedTx() : could not decode a denemecoin tx");
    int nPrevHeight;
    int nDepth;
    int64 nNetFee;
...
bool
CDenemecoinHooks::ConnectInputs (DatabaseSet& dbset,
                               map<uint256, CTxIndex>& mapTestPool,
                               const CTransaction& tx,
                               vector<CTransaction>& vTxPrev,
                               vector<CTxIndex>& vTxindex,
                               ...
                               bool fBlock, bool fMiner)
{
    ...
    if (tx.nVersion != DENEMECOIN_TX_VERSION)
    {
        ...
        if (foundOuts && (!fBlock || doStrictChecks (pindexBlock->nHeight)))
            return error("ConnectInputHook: non-Denemecoin tx has name outputs");

        // Make sure name-op outputs are not spent by a regular transaction, or the name
        // would be lost
        if (found)
            return error("ConnectInputHook() : a non-denemecoin transaction with a denemecoin input");
        return true;
    }
    ...
    if (!good)
        return error("ConnectInputsHook() : could not decode a denemecoin tx");
    ...
    switch (op)
    {
        case OP_NAME_NEW:
            if (found)
                return error("ConnectInputsHook() : name_new tx pointing to previous denemecoin tx");
...
bool
CDenemecoinHooks::DisconnectInputs (DatabaseSet& dbset, const CTransaction& tx,
                                  CBlockIndex* pindexBlock)
{
    if (tx.nVersion != DENEMECOIN_TX_VERSION)
        return true;
    ...
    bool good = DecodeNameTx(tx, op, nOut, vvchArgs, pindexBlock->nHeight);
    if (!good)
        return error("DisconnectInputsHook() : could not decode denemecoin tx");
    if (op == OP_NAME_FIRSTUPDATE || op == OP_NAME_UPDATE)
    {
        //vector<CDiskTxPos> vtxPos;
        vector<CNameIndex> vtxPos;
...
bool CDenemecoinHooks::CheckTransaction(const CTransaction& tx)
{
    if (tx.nVersion != DENEMECOIN_TX_VERSION)
        return true;
...
bool CDenemecoinHooks::ExtractAddress(const CScript& script, string& address)
{
...
bool
CDenemecoinHooks::ConnectBlock (CBlock& block, DatabaseSet& dbset,
                              CBlockIndex* pindex)
{
    return true;
}
...
bool
CDenemecoinHooks::DisconnectBlock (CBlock& block, DatabaseSet& dbset,
                                 CBlockIndex* pindex)
{
    return true;
}
...
bool CDenemecoinHooks::GenesisBlock(CBlock& block)
{
    if (fTestNet)
        return false;
...
int CDenemecoinHooks::LockinHeight()
{
    if (fTestNet)
        return 0;
...
bool CDenemecoinHooks::Lockin(int nHeight, uint256 hash)
{
    if (!fTestNet)
...
string CDenemecoinHooks::IrcPrefix()
{
    return "denemecoin";
}
