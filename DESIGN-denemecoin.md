...
The name\_firstupdate transaction has a network fee.  The network fees represents denemecoins (DC) that are destroyed.  This is in addition to the bitcoin-like miner transaction fees.
...
* Network fees start out at 50 DC per operation at the genesis block
* Every block, the network fees decreases based on this algorithm, in 1e-8 DC:
...
* As 50 DC are generated per block, the maximum number of registrations in the first 8192 blocks is therefore 2/3 of 8192, which is 5461
...
* if the transaction version does not indicate denemecoin, no inputs can be denemecoin outputs (i.e. have denemecoin transaction id and have a denemecoin script)
* if the transaction version does not indicate denemecoin, terminate with success
