...
bool AppInit2(int argc, char* argv[])
{
    ...
    if (mapArgs.count("-?") || mapArgs.count("--help"))
    {
        string strUsage = string() +
          _("denemecoin version") + " " + FormatFullVersion() + "\n\n" +
          _("Usage:") + "\t\t\t\t\t\t\t\t\t\t\n" +
            "  denemecoin [options]                   \t  " + "\n" +
            "  denemecoin [options] <command> [params]\t  " + _("Send command to -server or denemecoind") + "\n" +
            "  denemecoin [options] help              \t\t  " + _("List commands") + "\n" +
            "  denemecoin [options] help <command>    \t\t  " + _("Get help for a command") + "\n";

        strUsage += "\n" + HelpMessage();

#if defined(__WXMSW__) && defined(GUI)
        // Tabs make the columns line up in the message box
        wxMessageBox(strUsage, "Denemecoin", wxOK);
#else
        // Remove tabs
...
//#if defined(__WXMSW__) && defined(GUI)
    ...
    if (psingleinstancechecker->IsAnotherRunning())
    {
        ...
        loop
        {
            ...
            HWND hwndPrev = FindWindowA("wxWindowClassNR", "Denemecoin");
            if (hwndPrev)
            {
    ...
    if (!lock.try_lock())
    {
        wxMessageBox(strprintf(_("Cannot obtain a lock on data directory %s.  denemecoin is probably already running."), GetDataDir().c_str()), "Denemecoin");
        return false;
    }
    ...
    if (!fNoListen)
    {
        ...
        {
            wxMessageBox(strErrors, "Denemecoin");
            return false;
        }
    }
    ...
    if (mapArgs.count("-mininput"))
    {
        ...
        {
            wxMessageBox(_("Invalid amount for -mininput=<amount>"), "Denemecoin");
            return false;
        }
    }
    ...
    if (!strErrors.empty())
    {
        wxMessageBox(strErrors, "Denemecoin", wxOK | wxICON_ERROR);
        return false;
    }
    ...
    if (mapArgs.count("-proxy"))
    {
        ...
        {
            wxMessageBox(_("Invalid -proxy address"), "Denemecoin");
            return false;
        }
    }
    ...
    if (mapArgs.count("-paytxfee"))
    {
        ...
        {
            wxMessageBox(_("Invalid amount for -paytxfee=<amount>"), "Denemecoin");
            return false;
        }
        if (nTransactionFee > 0.25 * COIN)
            wxMessageBox(_("Warning: -paytxfee is set very high.  This is the transaction fee you will pay if you send a transaction."), "Denemecoin", wxOK | wxICON_EXCLAMATION);
    }
    ...
    if (!CreateThread(StartNode, NULL))
        wxMessageBox("Error: CreateThread(StartNode) failed", "Denemecoin");
...
std::string HelpMessage()
{
    std::string strUsage = std::string(_("Options:\n")) +
        "  -conf=<file>     \t\t  " + _("Specify configuration file (default: denemecoin.conf)\n") +
        "  -pid=<file>      \t\t  " + _("Specify pid file (default: denemecoind.pid)\n") +
...
#ifdef USE_SSL
    strUsage += std::string() +
        _("\nSSL options: (see the denemecoin Wiki for SSL setup instructions)\n") +
