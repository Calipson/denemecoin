...
#else
    const char* pszModule = "denemecoin";
#endif
    if (pex)
...
#ifdef GUI
    if (wxTheApp && !fDaemon)
        MyMessageBox(pszMessage, "Denemecoin", wxOK | wxICON_ERROR);
#endif
    throw;
}
...
#ifdef GUI
    uiInterface.ThreadSafeMessageBox(strMessage, "Denemecoin", wxOK | wxICON_EXCLAMATION);
#else
    ThreadSafeMessageBox(strMessage, "Denemecoin", wxOK | wxICON_EXCLAMATION);
#endif
...
string GetDefaultDataDir()
{
    ...
    // Windows: C:\Documents and Settings\username\Application Data\Denemecoin
    // Mac: ~/Library/Application Support/Denemecoin
    // Unix: ~/.denemecoin
#ifdef __WXMSW__
    // Windows
...
{
    string confFile = GetConfigFile("denemecoin.conf");
...
{
    namespace fs = boost::filesystem;
    fs::path pathConfig(GetArg("-pid", "denemecoind.pid"));
    if (!pathConfig.is_complete())
        pathConfig = fs::path(GetDataDir()) / pathConfig;
    return pathConfig.string();
}
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct.  If your clock is wrong Denemecoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    boost::thread(boost::bind(MyMessageBox, strMessage+" ", string("Denemecoin"), wxOK | wxICON_EXCLAMATION, (wxWindow*)NULL, -1, -1));
                }
            }
        }
